...
// Navigation icons — inline SVG use elements (same as FontAwesomeIcon component)
const FAIcon = ({ icon }: { icon: string }) => (
  <svg className="font-awesome inline-block fill-current leading-none">
    <use xlinkHref={`/sprites/font-awesome/solid.svg#${icon}`} />
  </svg>
);

// NavigationItem — static version (no useLocation, no Link)
const NavItem = ({ href, icon, text, isActive }: { href: string; icon: React.ReactNode; text: string; isActive?: boolean }) => (
  <li>
    <a
      ...
      href={href}
    >
      <span className="relative text-xl lg:pr-2 lg:text-3xl">{icon}</span>
...
// Full page layout matching AppPage + Navigation (non-logged-in state) + TermPage
const TermsFullPage = () => {
  return (
          ...
          <nav className="border-cax-border bg-cax-surface fixed right-0 bottom-0 left-0 z-10 h-12 border-t lg:relative lg:h-full lg:w-48 lg:border-t-0 lg:border-r">
            <div className="relative grid grid-flow-col items-center justify-evenly lg:fixed lg:flex lg:h-full lg:w-48 lg:flex-col lg:justify-between lg:p-2">
              <ul className="grid grid-flow-col items-center justify-evenly lg:grid-flow-row lg:auto-rows-min lg:justify-start lg:gap-2">
                <NavItem href="/" icon={<FAIcon icon="home" />} text="ホーム" />
                <NavItem href="/search" icon={<FAIcon icon="search" />} text="検索" />
                <NavButton icon={<FAIcon icon="sign-in-alt" />} text="サインイン" />
                <NavItem href="/terms" icon={<FAIcon icon="balance-scale" />} text="利用規約" isActive />
              </ul>
            </div>
          </nav>
        </aside>
        <main className="relative z-0 w-screen max-w-screen-sm min-w-0 shrink pb-12 lg:pb-0">
          <article className="px-2 pb-16 leading-relaxed md:px-4 md:pt-2">
            ...
            <ol className="list-decimal pl-8">
              ...
              <li>
                ユーザーは、いかなる場合にも、ユーザーIDおよびパスワードを第三者に譲渡または貸与し、もしくは第三者と共用することはできません。当社は、ユーザーIDとパスワードの組み合わせが登録情報と一致してサインインされた場合には、そのユーザーIDを登録しているユーザー自身による利用とみなします。
              </li>
              <li>
...
export function getTermsSSRHtml(): string {
  if (cached === null) {
    cached = renderToString(<TermsFullPage />);
  }
  return cached;
}